...
{   
    ...
    for (const auto &accessPoints : pinAccessPoints)
        for (const auto &point : accessPoints)
            boundingBox.Update(point);
    // if (name == "issue_stage_i/i_issue_read_operands/i_ariane_regfile/n_132")
    // {
    //     std::cout << "o:" << std::endl;
    //     for (auto a : _pinAccessPoints)
    //     {
    //         std::cout << std::get<0>(a[0]) << " " << std::get<1>(a[0]) << " " << std::get<2>(a[0]) << std::endl;
    //     }
    //     std::cout << "net:" << std::endl;
    //     for (auto b : pinAccessPoints)
    //     {
    //         for (auto c : b)
    //         {
    //             std::cout << c.layerIdx << " " << c.x << " " << c.y << std::endl;
    //         }
    //     }
